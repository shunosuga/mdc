...
def extend_to_word_boundary(tokens: list[str], labels: list[int], 
                          original_text: str, token_spans: list[dict]) -> list[int]:
    """
        ...
        if labels[i] == 1:
            ...
            if end_idx < len(tokens):
                ...
                while (end_idx < len(tokens) and 
                       current_char_end < len(original_text) and
                       not original_text[current_char_end].isspace() and
                       original_text[current_char_end].isalnum()):
...
class RestorationTester:
    ...
    def test_sample(self, sample: dict[str, Any]) -> dict[str, Any]:
        ...
        recall = len(intersection) / len(expected_set) if expected_set else 1.0 if not restored_set else 0.0
    ...
    def test_with_examples(self, samples: list[dict[str, Any]], num_examples: int = 5) -> dict[str, Any]:
        """
        ...
        failed_examples = []
        for sample in samples[:num_examples * 10]:  # Look at more samples to find failures
        ...
        for sample in samples[:num_examples * 10]:
            result = self.test_sample(sample)
            if result["exact_match"] and result["expected"] and len(success_examples) < num_examples:
